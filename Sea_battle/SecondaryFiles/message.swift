...
enum Message: String {
    case firstLaunch = "запуск", separator = "–", rules = "правила", commands = "команды", errorCommand = "ошибка ввода",
         ...
         wounded = "попадание", enterCommand = "ввод команды", leaveCurrentGame = "перезапуск"
...
1. Откройте настройки терминала (вкладка предпочтения/Preferences), далее перейдите в профили/Profiles.
   Во вкладке текст/Text, в подразделе шрифт/Font нажмите кнопку изменить/Change.

2. Установите рекомендуемые значения для следующих параметров:
